#!/usr/bin/env bash
# Run on the Gitea/dev side. Publish a clean release SNAPSHOT to the public GitHub repo.
# - Snapshot only (NO Gitea git history reaches GitHub).
...
# Usage: publish-from-gitea.sh vX.Y.Z [--dry-run]
set -euo pipefail
VERSION="${1:?usage: publish-from-gitea.sh vX.Y.Z [--dry-run]}"
