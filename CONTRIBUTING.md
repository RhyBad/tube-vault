...
TubeVault is developed **upstream on a private Gitea instance**. This GitHub repository is a **publish
mirror**: clean release snapshots are pushed here, and GitHub Actions builds the public Docker image (GHCR)
and creates releases. **Development does not happen on GitHub**, and code flows one-way (Gitea → GitHub).

## Code contributions — not accepted
Because code is one-directional (Gitea is the source of truth) and development is fast-moving / AI-assisted,
