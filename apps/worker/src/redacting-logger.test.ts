/**
 * Unit: the worker's RedactingConsoleLogger masks registered secrets in every
...
const SECRET = 'worker-logger-secret-123321';
...
describe('RedactingConsoleLogger (worker)', () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it('masks a registered secret in error() output (stderr), params included', () => {
    ...
    new RedactingConsoleLogger('spec').error('job failed', `stderr tail with ${SECRET}`);
    const out = writes.join('');
    expect(out).toContain('***REDACTED***');
    expect(out).not.toContain(SECRET);
  });
